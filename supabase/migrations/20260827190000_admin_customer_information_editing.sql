-- Allow authenticated owner/staff admins to correct quote customer information.
-- The RPC updates the quote record, keeps linked customer files aligned,
-- and writes an audit event without exposing direct table updates to the browser.

create or replace function public.update_quote_customer_information_admin(
...
begin
  ...
  if length(v_company_name) > 200 then
  ...
  v_next_quote_data := coalesce(v_previous.quote_data, '{}'::jsonb)
    - 'companyName'
    - 'company_name'
    - 'businessName'
    - 'business_name';
  ...
  if v_company_name <> '' then
    v_next_quote_data := jsonb_set(v_next_quote_data, '{companyName}', to_jsonb(v_company_name), true);
  end if;
  ...
  if v_previous_company is distinct from v_company_name then
  ...
  if coalesce(array_length(v_changed_fields, 1), 0) > 0 then
    ...
    values (
      ...
      'Customer information updated by ' || coalesce(v_actor, 'Admin') || ': ' || array_to_string(v_changed_fields, ', ')
    );
  end if;
  ...
  select
    qr.id,
    qr.customer_name,
    coalesce(qr.quote_data ->> 'companyName', ''),
    qr.customer_email,
    qr.customer_phone,
...
revoke all on function public.update_quote_customer_information_admin(uuid, text, text, text, text, text) from public;
revoke execute on function public.update_quote_customer_information_admin(uuid, text, text, text, text, text) from anon;
grant execute on function public.update_quote_customer_information_admin(uuid, text, text, text, text, text) to authenticated;

comment on function public.update_quote_customer_information_admin(uuid, text, text, text, text, text)
is 'Owner/staff admin RPC for audited customer information corrections on quote requests.';
