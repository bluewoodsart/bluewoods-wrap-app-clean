...
begin
  ...
  if current_rep_id is null then
    raise exception 'Active rep access is required.';
  end if;
  ...
  where rpi.submitted_by_admin_user_id = current_rep_id
...
begin
  ...
  if p_is_featured is true then
    ...
    where submitted_by_admin_user_id = current_rep_id
      and id <> p_idea_id;
  end if;
  ...
  where id = p_idea_id
    and submitted_by_admin_user_id = current_rep_id
