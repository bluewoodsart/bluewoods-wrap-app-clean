-- Reps can send an idea straight to the build lane by including the word "build".
-- Jarrel remains auto-approved as the original test lane.
...
begin
  ...
  normalized_idea := trim(coalesce(p_idea_text, ''));

  if length(normalized_idea) < 40 then
  ...
  next_status := case
    ...
    when normalized_idea ~* '\mbuild\M' then 'approved'
    else 'pending_review'
  end;
