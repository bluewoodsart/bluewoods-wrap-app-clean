...
const requestsBuildLane = (value: string) => /\bbuild\b/i.test(value);
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const isBuildLane = isJarrelTestLane || requestsBuildLane(direction);
  ...
  const subject = isBuildLane
    ? `[Rep Build Lane] ${repName} page idea`
    ...
    <div style="margin:0;background:#f8fafc;padding:24px;font-family:Arial,sans-serif;line-height:1.5;color:#111827;">
      <div style="max-width:720px;margin:0 auto;">
        <div style="padding:24px;border-radius:16px;background:#111827;color:#ffffff;">
          <p style="margin:0 0 8px;color:#93c5fd;font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">${isBuildLane ? 'Rep Page Build Lane' : 'Rep Cover Page Review'}</p>
          <h1 style="margin:0;font-size:25px;">${escapeHtml(repName)} submitted page direction.</h1>
          <p style="margin:10px 0 0;color:#d1d5db;">${isBuildLane ? 'This idea is approved for a test build. Build, test, correct, and record the look.' : 'Review this before approving any live front-end change.'}</p>
        </div>
        ...
        <div style="margin-top:18px;padding:18px;border-radius:14px;background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;">
          <p style="margin:0;font-weight:700;">${isBuildLane ? 'Build lane requested' : 'Approval step required'}</p>
          <p style="margin:8px 0 0;">${isBuildLane ? `Codex can build/test this page idea, make corrections, and show the result before treating it as final. Page path: ${escapeHtml(pageUrl)}.` : `Codex should summarize this recommendation and wait for owner approval before updating ${escapeHtml(pageUrl)}.`}</p>
        </div>
      </div>
    </div>
  `;
  ...
  try {
    ...
    await sendEmail(apiKey, {
      ...
      subject,
      html,
      text: `Rep page direction submitted.\n\nRep: ${repName} (${repSlug})\nEmail: ${repEmail || 'Not provided'}\nPage: ${pageUrl}\n\nSubmitted direction:\n${direction}\n\n${isBuildLane ? 'Build lane: approved for build/test/correction. Show the look before finalizing.' : 'Approval step required before updating the live page.'}`
    });

    if (isBuildLane && repEmail) {
      await sendEmail(apiKey, {
        from: FROM_EMAIL,
        to: repEmail,
        reply_to: REVIEW_EMAIL,
        subject: 'Your page idea is in the build lane',
        html: `
          <div style="margin:0;background:#f8fafc;padding:24px;font-family:Arial,sans-serif;line-height:1.5;color:#111827;">
            <div style="max-width:680px;margin:0 auto;">
              <div style="padding:22px;border-radius:16px;background:#111827;color:#ffffff;">
                ...
                <p style="margin:10px 0 0;color:#d1d5db;">This one is in the build lane, so BWB can build a first look and make corrections.</p>
              </div>
              <div style="margin-top:18px;padding:20px;border-radius:14px;background:#ffffff;border:1px solid #e5e7eb;">
                <p style="margin:0 0 12px;">Keep testing this flow from your rep portal. For each page idea, send:</p>
        ...
        `,
        text: [
          'We received your page idea and it is in the build lane.',
          '',
