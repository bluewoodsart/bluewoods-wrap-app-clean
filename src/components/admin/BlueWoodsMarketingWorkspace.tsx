...
const MasterFrontEnd = ({ onOpenSeo, onOpenBrandAssets, onOpenContent }: { onOpenSeo: () => void; onOpenBrandAssets: () => void; onOpenContent: () => void }) => (
  <div className="space-y-5">
    ...
    <Card className="overflow-hidden border-violet-300 shadow-sm">
      <CardHeader className="border-b border-violet-900 bg-gradient-to-r from-violet-950 via-slate-950 to-fuchsia-950 text-white">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <StatusPill label="Live" tone="live" />
              <span className="text-xs font-bold uppercase tracking-[0.12em] text-violet-200">Continuously updated build archive</span>
            </div>
            <CardTitle className="mt-3 text-2xl text-white">Blue Woods Labs Master Front End</CardTitle>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-300">The broader Blue Woods front-end collection where your newest builds, public experiences, testing program, and project launches continue to grow.</p>
          </div>
          <Button asChild className="shrink-0 bg-violet-300 text-slate-950 hover:bg-violet-200">
            <a href="https://master-front-end-blue-woods.bluewoodsart.chatgpt.site/" target="_blank" rel="noreferrer"><Maximize2 className="mr-2 h-4 w-4" />Open Blue Woods Master</a>
          </Button>
        </div>
      </CardHeader>
      <CardContent className="p-3 sm:p-5">
        <div className="overflow-hidden rounded-2xl border border-violet-300 bg-slate-100 shadow-inner">
          <div className="flex items-center gap-2 border-b border-violet-200 bg-white px-4 py-2.5">
            <div className="flex gap-1.5" aria-hidden="true"><span className="h-2.5 w-2.5 rounded-full bg-red-400" /><span className="h-2.5 w-2.5 rounded-full bg-amber-400" /><span className="h-2.5 w-2.5 rounded-full bg-emerald-400" /></div>
            <div className="ml-2 flex min-w-0 flex-1 items-center gap-2 rounded-lg bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-900"><Globe2 className="h-3.5 w-3.5 shrink-0" /><span className="truncate">master-front-end-blue-woods.bluewoodsart.chatgpt.site</span></div>
          </div>
          <div className="relative aspect-[16/10] min-h-[360px] bg-slate-950">
            <iframe
              src="https://master-front-end-blue-woods.bluewoodsart.chatgpt.site/"
              title="Live Blue Woods Labs master front end"
              loading="lazy"
              sandbox="allow-forms allow-popups allow-scripts allow-same-origin"
              referrerPolicy="strict-origin-when-cross-origin"
              className="absolute inset-0 h-full w-full border-0"
        ...
        <p className="mt-3 text-xs leading-5 text-slate-500">This second live preview remains separate from the SlapWrapz production website above. Use it as the master view of the newer Blue Woods front ends you continue building.</p>
      </CardContent>
    </Card>
